...
{ 
	public class CameraController : MonoBehaviour
	{
		...
		private bool isZoomingOnWinner = false;

		public void ZoomOnWinner(Transform winner)
		{
			target = winner;
			isZoomingOnWinner = true;
			// Khởi tạo camOffset chuẩn để bắt đầu zoom
		...
		void Update()
		{
			if (isZoomingOnWinner)
			{
				// Xoay chậm quanh người thắng
				transform.position = target.position;
				transform.Rotate(Vector3.up * 30f * Time.deltaTime);

				// Zoom lại gần
				camOffset.z = Mathf.MoveTowards(camOffset.z, -3f, 5f * Time.deltaTime);
				camOffset.y = Mathf.MoveTowards(camOffset.y, 1.5f, 5f * Time.deltaTime);
				
				camPos.localPosition = camOffset;
				camPos.LookAt(target.position + Vector3.up * 1f); // Nhìn thẳng vào mặt
				return;
			}
